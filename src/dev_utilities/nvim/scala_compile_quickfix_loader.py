...
def run():
    ...
    input_file = f"{os.environ.get('PROJECT_PATH')}/{os.environ.get('PROJECT_FILE_TO_READ')}"
    if os.path.exists(input_file):
        log = open(input_file).readlines()
        error_prefix = "[error] "
        errorLineA = re.compile(r"-- Error: (?P<filePath>.+):(?P<line>\d+):(?P<column>\d+)")
        errorLineB = re.compile(r"-- \[(?P<errorCode>E\d+)\] (.+): (?P<filePath>.+):(?P<line>\d+):(?P<column>\d+)")
        ...
        for l in result:
            ...
            if m:
                if creatingEntry:
                    ...
                    current_entry = ErrorItem(m.group("filePath"), m.group("line"), m.group("column"))
                else:
                    creatingEntry = True
                    current_entry = ErrorItem(m.group("filePath"), m.group("line"), m.group("column"))
            else:
                if creatingEntry:
                    current_entry = replace(current_entry, **{"message": f"{current_entry.message}{l}"})
